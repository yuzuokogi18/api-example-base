...
export class UserRepository {
  ...
  public static async findAll(): Promise<User[]> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT user_id, role_id, name, address, phone_number, age, username, password, created_at, updated_at, deleted FROM users', (error: any, results) => {
        if (error) {
          reject(error);
        } else {
  ...
  public static async findByUsername(username: string): Promise<User | null> {
    return new Promise((resolve, reject) => {
      connection.query('SELECT * FROM users WHERE username = ?', [username], (error: any, results) => {
        if (error) {
          reject(error);
        } else {
          const users: User[] = results as User[];
  ...
  public static async createUser(user: User): Promise<User> {
    const query = 'INSERT INTO users (name, address, phone_number, age, role_id, username, password, created_at, updated_at, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
    return new Promise((resolve, reject) => {
  ...
  public static async updateUser(user_id: number, userData: User): Promise<User | null> {
    const query = 'UPDATE users SET name = ?, address = ?, phone_number = ?, age = ?, role_id = ?, username = ?, password = ?, updated_at = ?, deleted = ? WHERE user_id = ?';
    return new Promise((resolve, reject) => {
